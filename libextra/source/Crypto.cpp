...
//See http://www.codeproject.com/Articles/19451/A-simple-MFC-class-to-encrypt-data-with-the-Crypto
...
CCrypto::CCrypto() : m_hCryptProv(NULL), m_hKey(NULL), m_hHash(NULL)
{
	...
	if(!::CryptAcquireContext(&m_hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) //CC 27/07/2015 - Added CRYPT_VERIFYCONTEXT (otherwise wouldn't work for non-admin users (no access to local key store))
		return;
